...
This is the compact decision index for choices that affect multiple
authorities. It records the intent, status and supersession relationship; the
domain contract, source owner and tests explain the full behaviour.

## How to use this ledger

Read the decisions as a connected contract, not as isolated preferences:

```mermaid
flowchart TB
    A["Decision ledger"] --> B["Authoritative topic contract"]
    B --> C["Source module ownership"]
    C --> D["Executable verification"]
    D --> E["Operator / research evidence"]
```

Each decision is a compact pointer to the larger explanation. Before changing
behaviour, locate the referenced topic document, source owner and tests; update
those authorities together. A later decision may supersede an earlier one, but
the supersession must be explicit so the implementation cannot accidentally
combine incompatible rules.

| Status | Meaning | Implementation treatment |
|---|---|---|
| `FROZEN` | Approved contract or engineering rule | Must be preserved unless explicitly superseded |
| `PROVISIONAL` | Agreed direction whose thresholds or details remain evidence-sensitive | Implement the boundary; keep calibration explicit |
| `FROZEN FOR INITIAL IMPLEMENTATION` | V1 behaviour approved for the first release | Treat as a hard acceptance contract |
| `SUPERSEDED` | Replaced by a named later decision | Follow the later decision and retain the link |

The ledger does not replace the domain documents. It answers *which choice is
intentional*; the domain document answers *how the choice works end to end*.

## Verification and evidence

Every material decision must be traceable to an authoritative topic document,
source/module owner and executable test or explicit live-evidence requirement.
The ledger itself proves governance traceability; it does not prove that a
broker, provider or research dataset has been exercised.
...
**Supersession note:** V1 now defines only a positive DEMO guard and intentionally does not specify REAL authorization/hard-block behaviour.
...
## DEC-063 — All maintained V1 project code follows a frozen lightweight production-code standard

**Decision:** All maintained V1 project code must follow `docs/60-engineering/CODING_STANDARD.md`, including runtime, broker, risk, execution, persistence, dashboard, operator, research, discovery, learning, scripts, configuration, migration, backup, restore and test code: Python 3.11+; standard-library-first/minimal runtime dependencies; official MetaTrader5 boundary; pure functions for deterministic calculations where practical; classes only for genuine state/resource/lifecycle ownership; typed dataclasses/enums/IDs where useful; one verified snapshot/shared derived facts rather than duplicate MT5 reads/calculations; no giant all-in-one file and no unnecessary micro-file/framework/factory/service-manager architecture; concise comments/docstrings explaining why/safety/chronology; explicit non-silent error handling; structured secret-safe logging; heavier research dependencies isolated; code-quality review at every phase exit. Scope-specific exceptions must be explicit and bounded; they cannot weaken safety, chronology, determinism, auditability or secret handling.
**Status:** FROZEN FOR INITIAL IMPLEMENTATION
...
the previous decision and explaining the change. Superseded entries remain
visible so the active contract cannot be reconstructed by guesswork.
