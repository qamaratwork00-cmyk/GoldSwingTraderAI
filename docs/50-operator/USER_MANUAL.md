...
```text
1. Open MT5 and verify the intended DEMO account.
...
If available under the frozen Risk Contract, manual reset is a deliberate governed action. It does not delete today's broker loss and cannot bypass technical/account/order blocks.
...
A normal trading decision such as `WAIT` is not a system problem.
