...
_FAMILY_SIGNATURES: dict[StrategyFamily, frozenset[ApprovedPrimitive]] = {
    StrategyFamily.TREND_PULLBACK_CONTINUATION: frozenset(
        {
            ApprovedPrimitive.STRUCTURE_TREND,
            ApprovedPrimitive.TECHNICAL_LOCATION,
            ApprovedPrimitive.EMA_FLOW,
            ApprovedPrimitive.ENTRY_TIMING,
            ApprovedPrimitive.TARGET_PATH,
        }
    ),
    StrategyFamily.BREAKOUT_EXPANSION: frozenset(
        {
            ApprovedPrimitive.STRUCTURE_BREAK,
            ApprovedPrimitive.DISPLACEMENT,
            ApprovedPrimitive.ATR_VOLATILITY,
            ApprovedPrimitive.TARGET_PATH,
        }
    ),
    StrategyFamily.BREAKOUT_RETEST_CONTINUATION: frozenset(
        {
            ApprovedPrimitive.STRUCTURE_BREAK,
            ApprovedPrimitive.TECHNICAL_LOCATION,
            ApprovedPrimitive.ENTRY_TIMING,
    ...
    StrategyFamily.LIQUIDITY_SWEEP_REVERSAL: frozenset(
        {
            ApprovedPrimitive.LIQUIDITY_SWEEP,
            ApprovedPrimitive.CANDLE_REJECTION,
            ApprovedPrimitive.MSS_SHIFT,
            ApprovedPrimitive.TECHNICAL_LOCATION,
        }
    ),
    StrategyFamily.FAILED_BREAKOUT_REVERSAL: frozenset(
        {
            ApprovedPrimitive.STRUCTURE_BREAK,
            ApprovedPrimitive.CANDLE_REJECTION,
            ApprovedPrimitive.MSS_SHIFT,
    ...
    StrategyFamily.COMPRESSION_EXPANSION: frozenset(
        {
            ApprovedPrimitive.COMPRESSION,
            ApprovedPrimitive.DISPLACEMENT,
            ApprovedPrimitive.ATR_VOLATILITY,
            ApprovedPrimitive.STRUCTURE_BREAK,
        }
...
def propose_candidate(
    ...
    *,
    existing: tuple[StrategyCandidate, ...] = (),
    ...
    required = tuple(item for item in ranked if counts[item] >= required_cutoff)[
        : cfg.maximum_required_primitives
    ]
    optional = tuple(
        item
        for item in ranked
        if item not in required and counts[item] >= optional_cutoff
    )[: cfg.maximum_optional_primitives]
    if len(required) < 2:
    ...
    recipe = CandidateRecipe(
        ...
        invalidation_model=(
            InvalidationModel.RECLAIM_FAILURE
            if ApprovedPrimitive.LIQUIDITY_SWEEP in required
            else InvalidationModel.STRUCTURAL
        ),
        target_model=(
            TargetModel.EXPANSION_PATH
            if ApprovedPrimitive.DISPLACEMENT in required
            or ApprovedPrimitive.COMPRESSION in required
            else TargetModel.STRUCTURAL_LIQUIDITY
        ),
    )
    ...
    if duplicate is not None:
        reason = (
            ...
            if duplicate.stage is CandidateStage.REJECTED
            else "DUPLICATE_EXISTING_CANDIDATE"
        )
...
def candidate_similarity(left: StrategyCandidate, right: StrategyCandidate) -> float:
    required = _jaccard(set(left.recipe.required), set(right.recipe.required))
    optional = _jaccard(set(left.recipe.optional), set(right.recipe.optional))
...
def _parent_family(
    observations: tuple[DiscoveryObservation, ...],
    required: frozenset[ApprovedPrimitive],
    cfg: DiscoveryConfig,
) -> StrategyFamily | None:
...
def _hypothesis(
    ...
    return (
        f"{trigger.value}: repeated {direction.value} episodes around {lineage} share "
        f"[{required}]; validate this declarative recipe before any production use."
    )
